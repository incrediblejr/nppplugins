...
//
#pragma warning(push)
//TreeListWnd.cpp(622) : warning C4312 : 'type cast' : conversion from 'unsigned int' to 'HTREEITEM' of greater size
//TreeListWnd.cpp(747) : warning C4244 : '=' : conversion from 'LRESULT' to 'int', possible loss of data
//TreeListWnd.cpp(2905): warning C4302: 'type cast': truncation from 'HTREEITEM' to 'unsigned int'
//TreeListWnd.cpp(6360) : warning C4310 : cast truncates constant value
//TreeListWnd.cpp(6372) : warning C4311 : 'type cast' : pointer truncation from 'HTREEITEM' to 'unsigned int'

#pragma warning(disable : 4456 4312 4244) // 4302 4310 4311)
...
#if _WIN64
typedef unsigned long long ItemType;
#define 	U(h)				((unsigned long long)(h))
#else
...
typedef struct
	...
	ItemType	uFirstChild;								// Ist die Nummer des ersten Kind-Eintrages (0=keines)
	ItemType	uLastChild;									// Ist die Nummer des letzen Kind-Eintrages (0=keines)
	ItemType	uPrevItem;									// Ist die Nummer des vorherigen Eintrages (0=keines)
	ItemType	uNextItem; 									// Ist die Nummer des nächsten Eintrages (0=keines)
	ItemType	uParent; 									// Ist die Nummer des Elterneintrages (0=Root)
...
typedef struct
	...
	ItemType   *pItemPos;									// Liste mit den Offsets der sichtbaren Einträge
	...
	ItemType	uSelectedBase;								// Ist der Eintrag ab dem gewählt wurde
	ItemType	uSelectedItem;								// Ist der Eintrag der gerade gewählt ist
	unsigned	uSelectedSub;								// Ist die Spalte die gerade gewählt ist
	ItemType	uFocusItem;									// Ist der Eintrag der einen leeren Focus hat
	unsigned	uFocusSub;									// Ist die Spalte die einen leeren Focus hat
	ItemType	uToolTipItem;								// Ist der ToolTip-Eintrag der gerade gewählt ist
	...
	ItemType	uEditItem;									// Ist der Eintrag der gerade editiert wird
...
static int				TreeListSelectItem     (TreeListData *pData, ItemType uItem,unsigned uSubItem,int iMode);
static int				TreeListGetItemRect    (TreeListData *pData, ItemType uItem,unsigned uFlags,RECT *pRect);
static int				TreeListStartNotifyEdit(TreeListData *pData, ItemType uItem,unsigned uSub,WPARAM wParam,LPARAM lParam);
...
//	Ergibt den Rückgabewert der WM_NOTIFY Nachrich
static void CallbackEntry(TreeListData *pData,BaseItem *pEntry, ItemType uItem,unsigned uFlags,int &iImage,unsigned &uTextSize,LPCTSTR &pText)
{
NMTVDISPINFO	sInfo;
...
//	uFlags		: Welche Daten sollen abgefragt werden
//	Ergibt den Rückgabewert der WM_NOTIFY Nachrich
static void CallbackExtra(TreeListData *pData,BaseItem *pEntry,ExtraItem *pExtra, ItemType uItem,unsigned uSub,unsigned uFlags,int &iImage,unsigned &uTextSize,LPCTSTR &pText)
{
NMTVDISPINFO	sInfo;
...
static HIMAGELIST CreateDragImage(TreeListData *pData, ItemType uItem,unsigned uSub)
{
ExtraItem  *pExtra;
BaseItem   *pEntry;
HIMAGELIST	hList;
...
static int UpdateRect(TreeListData *pData, ItemType uItem,unsigned uSub)
{
BaseItem   *pEntry;
RECT		sRect;
unsigned	uPos;
...
//	uItem		: Ist die Nummer des Eintrages
//	Ergibt 1 wenn der Eintrag sichtbar war
static int UpdateRow(TreeListData *pData, ItemType uItem)
{
BaseItem   *pEntry;
RECT		sRect;
unsigned	uPos;
...
static void UpdateToolTip(TreeListData *pData, ItemType uItem,unsigned uFlags)
{
...
static void UpdateItems(TreeListData *pData, ItemType uItem)
{
unsigned	uPos;
unsigned	uOld;
unsigned	uNum;
...
static int TreeListEnsureVisible(TreeListData *pData, ItemType uItem,unsigned uSub)
{
BaseItem   *pEntry;
BaseItem   *pTemp;
ItemType	uTemp;
...
static int TreeListIsVisible(TreeListData *pData, ItemType uItem,unsigned uSub)
{
BaseItem   *pEntry;
unsigned	uPos;
int			iNum;
...
static int TreeListDeleteItem(TreeListData *pData, ItemType uItem,int iMode)
{
NMTREEVIEW	sNotify;
ExtraItem **pList;
ExtraItem  *pExtra;
...
static int  TreeListSetFocus(TreeListData *pData, ItemType uItem,unsigned uSub)
{
ExtraItem  *pExtra;
BaseItem   *pEntry;
BaseItem   *pTemp;
...
static int TreeListSelectChilds(TreeListData *pData, ItemType uItem,int iMode)
{
BaseItem   *pEntry;
unsigned	uLevel;
unsigned	uXor;
...
static unsigned TreeListSetInsertMark(TreeListData *pData, ItemType uItem,int iMode)
{
...
//	Ergibt die alte Farbe oder TV_NOCOLOR wenn keine Farbe eingestellt war
static COLORREF TreeListSetItemColor(TreeListData *pData, ItemType uItem,unsigned uSub,COLORREF uColor,int iMode)
{
TV_ITEM		sSet;
COLORREF	uOld;
ExtraItem  *pExtra;
...
static int TreeListSetTrackItem(TreeListData *pData, ItemType uItem,unsigned uSub)
{
ExtraItem	   *pExtra;
BaseItem	   *pEntry;
int				iRet=1;
...
static unsigned TreeListFindItem(TreeListData *pData, ItemType uItem,TVFIND *pFind)
{
int				iImage;
BaseItem	   *pEntry;
ExtraItem	   *pExtra;
...
static unsigned TreeListNextSelUntil(TreeListData *pData, ItemType uItem, ItemType uStop)
{
BaseItem	   *pEntry;
...
//	uStop	: Bestimmt bei welchem Eintrag abgebrochen werden soll werden soll
//	Ergibt die Nummer des Eintrages oder 0 wenn keiner gefunden wurde
static unsigned TreeListNextUnselUntil(TreeListData *pData, ItemType uItem, ItemType uStop)
{
BaseItem	   *pEntry;
...
int				iAdd;
	...
	if(uItem)												// Wurde auf einen Eintrag getrückt
		...
		if(pData->uStyle&TVS_FULLROWSELECT)
			...
			uMaskSub  =	TVHT_ONSUBICON | (pData->aColumn[TVHT_SUBTOCOL(sInfo.flags)].bEdit==TVAX_CHECK ? 0 : TVHT_ONSUBLABEL);
...
//	Ergibt das Handle des Edit-Controlls oder NULL bei einem Fehler
static HWND TreeListEditLabel(TreeListData *pData, ItemType uItem,unsigned uSub)
{
HDC				hDc;
HWND			hWnd;
LRESULT			lRet;
...
INT				iSel;
	...
	uBits |= U(lRet)&(TVIR_EDITCOMBOBOX|TVIR_EDITCOMBOLIST|TVIR_EDITFULL);
	...
	if(lRet&TVIR_EDITCOMBOBOX)								// Die Combobox füllen
		...
		SendMessage(hWnd,CB_SETEDITSEL,0,pData->uLastSel);	// Textauswahl weiderherstellen
		}
...
NoRootLines:
		...
		if(0 && uBits&(TVIS_SELECTED|TVIS_DROPHILITED|TVIS_UNTERLINE|TVIS_TRACKED|TVIS_TEXTCOLOR|TVIS_FOCUSED))
			...
			#pragma warning(suppress: 4127)
			if(0 &&(uBits&TVIS_SELECTED) && pData->cHasFocus && uItem==pData->uSelectedItem && !pData->uSelectedSub)
				{
		...
		for(uExtra=1;uExtra<=pData->uColumnCount;uExtra++)		// Extra Spalten zeichnen
			...
			if(!pExtra)											// Leeres Feld zeichnen
				...
				if(uBits&(TVIS_SELECTED|TVIS_DROPHILITED|TVIS_UNTERLINE|TVIS_TRACKED|TVIS_TEXTCOLOR|TVIS_FOCUSED))
					...
					// Ist das Feld ohne Focus ausgewählt
					#pragma warning(suppress: 4127)	
					if(0 &&(uBits&TVIS_SELECTED) && pData->cHasFocus && uItem==pData->uSelectedItem)
						{
						uTempColor = GetSysColor(COLOR_HIGHLIGHTTEXT);
						SelectObject(hDc,GetSysColorBrush(COLOR_HIGHLIGHT));
						Rectangle   (hDc,sButton.left-2,sButton.top-1,sButton.right+2,sButton.bottom+1);
						...
						if(uBits&TVIS_DROPHILITED) // FREEN 
							{
							uTempColor =     GetSysColor     (COLOR_HIGHLIGHTTEXT);
							SelectObject(hDc,GetSysColorBrush(COLOR_HIGHLIGHT   ));
							SelectObject(hDc,GetStockObject  (NULL_PEN          ));
