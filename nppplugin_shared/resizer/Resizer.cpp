...
{
  Clear();
  m_vInfo = new CControlInfo[nSize];	// alloc 23
  m_nSize = nSize;
...
int CResizer::GetCoordinate(ESize eType, const RECT &rcInitial, const CBorder &border, const RECT &rc) const{
  ...
  case eFixed:
    {
      ...
      RECT frc;
      GetInitialDlgItemRect(border.nRelID, frc);
      int nRelOld = GetRectCoord(border.eRelType, frc);
  ...
  ASSERT(FALSE); //Wrong relation type is specified. Use items from EBorder enum.
  return 0;
}
...
int CResizer::GetRectCoord(ESize eType, const RECT &rc){
  ...
  ASSERT(FALSE); //Wrong side is specified. Use items from ESize enum.
  return 0;
}
...
int CResizer::Find(int nID) const{
  ...
  ASSERT(FALSE); //Possibly control id nID wasn't defined before it is used
